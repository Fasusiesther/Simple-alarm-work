...
root.geometry('450x400')  # width and height of the window
root.resizable(0, 0)  # sets fix size of window
root.title(' Alarm Update Clock')  # gives the window a title


addTime = Label(root, fg="red", text="Hour     Min     Sec",
                font='arial 15 bold').place(x=210)
setYourAlarm = Label(root, text="Set Time(24hrs): ",
                     bg="Pink", font="arial 11 bold").place(x=80, y=50)
hour = StringVar()
min = StringVar()
sec = StringVar()
...
def alarm():
    while True:
        ...
        if actual_time == set_alarm_time:
            ...
            messagebox.showinfo("TIME'S UP, START OVER!!!")


# create a button to set the alarm
submit = Button(root, text="Set Alarm Clock", fg="red", width=20,
                command=Threading, font=("arial 20 bold")).pack(pady=80, padx=120)
# run the program
root.mainloop()
